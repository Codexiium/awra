...
import { Heart, ShoppingBag, Check, ShieldCheck, Truck, Ruler, Star } from "lucide-react";
...
export default function ProductDetailPage(props: PageProps<"/product/[slug]">) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <nav className="text-xs font-mono text-zinc-500 uppercase tracking-widest mb-8 flex items-center gap-2">
        ...
        <span>/</span>
        <Link href={`/category/${product.category.toLowerCase()}`} className="hover:text-white transition-colors">
          {product.category}
        </Link>
        <span>/</span>
        ...
        <div className="lg:col-span-5 flex flex-col justify-between space-y-8">
          <div>
            ...
            <div className="flex items-baseline gap-3 mb-6 pb-6 border-b border-white/10">
              <span className="text-2xl font-mono font-bold text-white">${product.price} USD</span>
              {product.compareAtPrice && (
                <span className="text-base font-mono text-zinc-500 line-through">
                  ${product.compareAtPrice} USD
                </span>
              )}
          ...
          {/* Guarantee Badges */}
          <div className="grid grid-cols-2 gap-4 p-4 bg-[#101010] border border-white/10 text-[11px] font-mono text-zinc-400">
            <div className="flex items-center gap-2">
              <Truck className="w-4 h-4 text-zinc-200 shrink-0" />
              <span>FREE EXPRESS DELIVERY OVER $250</span>
            </div>
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-zinc-200 shrink-0" />
        ...
        <div className="max-w-3xl text-xs sm:text-sm font-sans leading-relaxed text-zinc-300">
          ...
          {activeTab === "shipping" && (
            <div className="space-y-3 font-mono text-xs text-zinc-300">
              ...
              <p>Express Air Freight: 2–4 Business Days ($25 USD or Free over $250 USD).</p>
